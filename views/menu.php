    ...
    <section class="apock-contenedor-comentarios mt-5">
      <div class="apock-area-comentar">
        ...
        <?php } else { ?>
          ...
          <form class="apock-inputs-comentarios" id="comentarioForm">
            <textarea name="" class="apock-area-comentario" id="nuevoComentario" disabled></textarea>
            <div class="apock-botones-comentar">
              <button class="apock-boton-enviar" type="submit" disabled>
          ...
          <?php if (isset($_SESSION['idvisitax'])) { ?>
              ...
              <form id="formulario-comentario" method="POST" action="../controllers/comentario.controller.php">
                  ...
                  <div class="mb-3 d-flex">
                      <img src="<?= $google_picture ?>" class="bolita" />
                      <textarea class="form-control" placeholder="Escribe tu comentario aquí..." id="tarea-comentario" name="comentario" required></textarea>
                  </div>
                  <button type="submit" class="btn btn-primary nnita">Guardar Comentario</button>
              </form>
          <?php } else { ?>
            <div class="teolvide">
              <h4>Inicie Sesión</h4>
              <p>Debe iniciar sesión para poder registrar un comentario.</p>
              <a href="../modelGmail/google-oauth.php" class="btn btn-primary nnita">Iniciar Sesión</a>
            </div>
          <?php } ?>
        </div>
      </div>
        ...
        function listarComent() {
          ...
          fetch(`../controllers/comentario.controller.php`, {
            ...
            .then(datosRecibidos => {
              ...
              let numFila = 1;
              $("#tabla-comentarios").innerHTML = '';
              datosRecibidos.forEach(registro => {
                let nuevafila = ``;
                const propietario = registro.idvisita == idpropietario;
                console.log('Es propietario:', propietario); 
                nuevafila = `
                <div class="porti">
                  <div class="ordenap">
                    <img src="${registro.picture}" class="bolita2" />
                    <div class="tuname">${registro.name_google}</div>
                    <div class="tudate">${registro.create_at}</div>
                    <div class="tudelete">${propietario ? `<button data-idcomentario="${registro.idcomentario}" class='eliminarp' type='button'></button>` : ''}</div>
                  </div>
                  <div class="d-flex tucomentariop">${registro.comentarios}</div>
                </div>
              `;

                $("#tabla-comentarios").innerHTML += nuevafila;
                numFila++;
              });
            })
            .catch(e => {
