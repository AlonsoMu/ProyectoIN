...
<head>
  ...
  <style>
     .mostrarResultados {
    background-color: #e6f0f6; /* color de fondo sombreado */
    border-radius: 5px; /* esquinas redondeadas */
    padding: 10px; /* espaciado interno */
    box-shadow: 0px 0px 5px 0px rgba(#e2e2e2); /* sombra */
  }
  .logo-negocio {
...
.nombre-negocio {
  margin-left: 10px; /* Espacio a la izquierda del nombre del negocio */
}
.resultado-item {
  margin-bottom: 10px; /* Espacio inferior entre cada resultado */
}

.resultado-item:hover {
  background-color: rgba(0, 0, 0, 0.1); /* Cambiar el color de fondo al pasar el mouse */
}
...
<body>
  ...
  <div id="resultados" class="container mt-3 justify-content-left" style="max-width: 700px;"></div>
  ...
  <script>
  // Agregar evento input al campo de búsqueda fuera de la función busquedaGeneral()
  $(document).ready(function() {
    $('#buscar').on('input', function() {
      const busqueda = $(this).val().trim(); // Obtener el valor del campo de búsqueda y eliminar espacios en blanco al inicio y al final
      if (busqueda !== '') { // Verificar si la búsqueda no está vacía
        // Realizar la búsqueda
        buscarNegocios(busqueda);
      } else {
        // Si la búsqueda está vacía, limpiar la lista de resultados
        limpiarResultados();
      }
    });
  });
  ...
  function buscarNegocios(busqueda) {
    ...
    parametros.append("operacion", "busquedaGeneral");
    ...
    fetch("./controllers/negocio.controller.php", {
      ...
      .then(datos => {
        ...
        // Actualizar la lista de negocios en la interfaz de usuario
        mostrarResultados(datos, busqueda);
      })
      .catch(error => {
  ...
  function mostrarResultados(datos, busqueda) {
    const resultadosElemento = document.getElementById("resultados");

    // Limpiar resultados anteriores
    resultadosElemento.innerHTML = "";

    // Filtrar y mostrar resultados solo si la búsqueda no está vacía
    if (busqueda !== '') {
        const resultadosFiltrados = datos.filter(registro => registro.nombre.toLowerCase().includes(busqueda.toLowerCase()));
        ...
        resultadosFiltrados.forEach(registro => {
            ...
            logoNegocio.src = `imgLogos/${registro.logo}`;
            ...
            resultadoItem.appendChild(logoNegocio);

            // Agregar nombre del negocio
            const nombreNegocio = document.createElement("span");
            nombreNegocio.textContent = registro.nombre;
            resultadoItem.appendChild(nombreNegocio);

            // Agregar resultado al contenedor
  ...
  campoBusqueda.addEventListener("input", function() {
    if (campoBusqueda.value.trim() !== '') {
      contenedorResultados.classList.add("mostrarResultados");
    } else {
      contenedorResultados.classList.remove("mostrarResultados");
    }
  });
});
</script>
